...
full-text (BM25), hybrid (RRF), and SQL retrieval over one copy of your data
on object storage.**

Most LlamaIndex vector-store integrations only expose the vector slice of
their engine. Infino keeps your data in Apache Parquet on object storage and
runs SQL, BM25, vector, and hybrid retrieval from a single in-process engine
— no separate search cluster to keep in sync. This package surfaces that
whole retrieval surface as a `BasePydanticVectorStore`, including a pure
BM25 mode, RRF hybrid in one SQL call, structured filters with the full
LlamaIndex operator surface, a text-pushdown pre-filter, and a SQL escape
hatch.
...
connection = infino.connect("./data")  # local path, S3 URI, or "memory://"
...
SQL call — no separate reranking round-trip. `MMR` re-embeds candidate
...
- `add(nodes) -> list[str]` — idempotent upsert by `node_id`.
